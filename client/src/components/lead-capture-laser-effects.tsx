...
// Card-level intensity scale — much more restrained than hero
const CARD_SCALE: Record<LaserIntensity, number> = {
  subtle:   0.14,
  standard: 0.20,
  high:     0.28,
};
...
function injectKeyframes() {
  ...
  style.textContent = `
    ...
    /* ── Card keyframes ─────────────────────────────────────────────────────── */
    @keyframes laser-card-border {
      0%   { background-position: -50% 0; }
      100% { background-position: 150% 0; }
    }
    @keyframes laser-card-diagonal {
    ...
    @media (prefers-reduced-motion: reduce) {
      ...
      .laser-card-sweep, .laser-card-diag {
...
// ─────────────────────────────────────────────────────────────────────────────
// LaserBorderSweep — subtle card-level laser layer
// Drop as the FIRST child inside any `relative overflow-hidden group` card.
// ─────────────────────────────────────────────────────────────────────────────
interface LaserBorderSweepProps {
  enabled?: boolean;
...
export function LaserBorderSweep({
  enabled,
  intensity = "standard",
  ...
  const s     = CARD_SCALE[intensity];

  const cornerMap: Record<string, { x: string; y: string }> = {
    "top-left":     { x: "0%",   y: "0%"   },
    "top-right":    { x: "100%", y: "0%"   },
    "bottom-left":  { x: "0%",   y: "100%" },
    "bottom-right": { x: "100%", y: "100%" },
  };
  const { x: cx, y: cy } = cornerMap[cornerPosition];

  const diagLeft = cornerPosition.includes("left") ? "8%" : undefined;
  const diagRight = cornerPosition.includes("right") ? "8%" : undefined;

  return (
    <div
      className="absolute inset-0 pointer-events-none overflow-hidden rounded-[inherit] opacity-60 group-hover:opacity-100 transition-opacity duration-300"
      style={{ zIndex: 0 }}
      aria-hidden="true"
    >
      {/* Corner radial glow */}
      <div
        style={{
          position: "absolute",
          top: cy === "0%" ? 0 : undefined,
          bottom: cy === "100%" ? 0 : undefined,
          left: cx === "0%" ? 0 : undefined,
          right: cx === "100%" ? 0 : undefined,
          width: "55%",
          height: "55%",
          background: `radial-gradient(ellipse at ${cx} ${cy}, ${color}${toHex2(op.glow * s * 1.1)} 0%, ${color}${toHex2(op.glow * s * 0.4)} 35%, transparent 65%)`,
          filter: "blur(2px)",
        }}
      />

      {/* Top border sweep — moving light along top edge */}
      <div
        className="laser-card-sweep"
        style={{
          position: "absolute",
          top: 0,
          left: 0,
          right: 0,
          height: "1px",
          background: `linear-gradient(90deg, transparent 0%, ${color}${toHex2(op.scan * s * 2.5)} 50%, transparent 100%)`,
          backgroundSize: "40% 100%",
          backgroundRepeat: "no-repeat",
          animation: `laser-card-border ${sweepDuration}s linear ${sweepDelay}s infinite`,
        }}
      />

      {/* Faint diagonal light streak */}
      <div
        className="laser-card-diag"
        style={{
          position: "absolute",
          top: "-15%",
          left: diagLeft,
          ...
          width: "2px",
          height: "130%",
          background: `linear-gradient(180deg, transparent 0%, ${color}${toHex2(op.beam * s * 0.7)} 30%, ${color}${toHex2(op.beam * s)} 55%, ${color}${toHex2(op.beam * s * 0.7)} 80%, transparent 100%)`,
          transform: `rotate(${cornerPosition.includes("right") ? "-" : ""}20deg)`,
          filter: "blur(0.5px)",
          animation: `laser-card-diagonal ${sweepDuration * 1.8}s ease-in-out ${sweepDelay * 0.5}s infinite`,
        }}
      />
    </div>
  );
...
// ─────────────────────────────────────────────────────────────────────────────
// LaserSectionGlow — ambient atmospheric glow behind a card grid section
// Wrap around a section's inner container div (max-w-* wrapper).
// ─────────────────────────────────────────────────────────────────────────────
interface LaserSectionGlowProps {
  enabled?: boolean;
  intensity?: LaserIntensity;
  preset?: LaserPreset;
  accentColor?: string;
...
export function LaserSectionGlow({
  enabled,
  intensity = "standard",
  preset = "performance-orange",
  accentColor,
  ...
  return (
    <div className={`relative ${className}`}>
      <div
        ...
        style={{
          background: `radial-gradient(ellipse at 50% ${yMap[glowPosition]}, ${color}${toHex2(op.glow * 0.09)} 0%, transparent 60%)`,
          zIndex: 0,
        }}
        aria-hidden="true"
      />
...
// ─────────────────────────────────────────────────────────────────────────────
// LeadCaptureLaserEffects — hero / success / divider variants
// ─────────────────────────────────────────────────────────────────────────────
...
export function LeadCaptureLaserEffects({
  ...
  return (
    ...
    >
      ...
      {/* Diagonal laser beam 1 — main */}
      <div
        style={{
          position: "absolute", top: "5%", left: "8%", width: "8px", height: "130%",
      ...
      <div
        style={{
          position: "absolute", top: "5%", left: "8%", width: "2.5px", height: "130%",
          ...
          transform: "rotate(25deg) translateX(2.75px)", opacity: 0.95,
        }}
      />
      ...
      <div
        style={{
          position: "absolute", top: "3%", right: "18%", width: "6px", height: "130%",
          ...
          transform: "rotate(-18deg)", filter: "blur(4px)",
          boxShadow: `0 0 14px 4px ${color}${toHex2(op.beam * 0.30)}`,
        }}
      />
      <div
        style={{
          position: "absolute", top: "3%", right: "18%", width: "2px", height: "130%",
          ...
          transform: "rotate(-18deg) translateX(-1px)", opacity: 0.85,
        }}
      />
      ...
      <div
        style={{
          position: "absolute", top: "8%", left: "42%", width: "4px", height: "120%",
          ...
          transform: "rotate(8deg)", filter: "blur(2.5px)",
      ...
      <div
        style={{
          position: "absolute", bottom: 0, left: "-10%", width: "75%", height: "55%",
      ...
      <div
        ...
        style={{
          position: "absolute", top: "20%", left: 0, width: "40%", height: "2px",
          ...
          animation: "laser-streak-1 6s ease-in-out 0s infinite", filter: "blur(0.5px)",
      ...
      <div
        ...
        style={{
          position: "absolute", top: "52%", left: 0, width: "32%", height: "1.5px",
      ...
      <div
        ...
        style={{
          position: "absolute", top: "36%", right: 0, width: "28%", height: "1.5px",
...
// ─────────────────────────────────────────────────────────────────────────────
// LaserUrgencyGlow — pulsing glow for urgency badge
// ─────────────────────────────────────────────────────────────────────────────
