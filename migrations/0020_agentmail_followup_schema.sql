...
BEGIN
  FOR r IN SELECT * FROM (VALUES
    ('agent_mail_followups_pkey',true,ARRAY['id']::text[],NULL::text),
    ('agent_mail_followups_org_id_id_unique',true,ARRAY['organization_id','id']::text[],NULL::text),
    ('agent_mail_followups_sequence_step_unique',true,ARRAY['organization_id','source_reply_queue_id','sequence_step']::text[],
    ...
    ('idx_followup_org_status_scheduled',false,ARRAY['organization_id','status','scheduled_for']::text[],NULL::text),
    ('idx_followup_inbox',false,ARRAY['organization_id','inbox']::text[],NULL::text),
    ('idx_followup_inbound',false,ARRAY['organization_id','source_inbound_message_id']::text[],NULL::text)
  ) expected(index_name,is_unique,key_columns,predicate)
  LOOP
    SELECT i.indisunique is_unique,i.indisvalid is_valid,i.indisready is_ready,
    ...
    IF NOT FOUND OR actual.is_unique<>r.is_unique OR NOT actual.is_valid OR NOT actual.is_ready
