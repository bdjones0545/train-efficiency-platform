...
export interface IStorage {
  ...
  updateCashoutStatusForOrganization(id: string, orgId: string, status: string): Promise<Cashout | undefined>;
  ...
  creditManualPaymentForOrganization(userId: string, orgId: string, amountCents: number, description: string, method: string): Promise<WalletTransaction | undefined>;
...
export class DatabaseStorage implements IStorage {
  ...
  async updateCashoutStatusForOrganization(id: string, orgId: string, status: string): Promise<Cashout | undefined> {
    const [updated] = await db
      .update(cashouts)
      ...
      .where(and(
        ...
        sql`EXISTS (
          SELECT 1 FROM coach_profiles cp
          WHERE cp.id = ${cashouts.coachId} AND cp.organization_id = ${orgId}
        )`,
      ))
      .returning();
    return updated || undefined;
  ...
  async creditManualPaymentForOrganization(userId: string, orgId: string, amountCents: number, description: string, method: string): Promise<WalletTransaction | undefined> {
    if (!Number.isSafeInteger(amountCents) || amountCents <= 0) return undefined;
    return db.transaction(async (trx) => {
      const membership = await trx.execute(sql`
        SELECT user_id FROM user_profiles
        WHERE user_id = ${userId} AND organization_id = ${orgId} AND role = 'CLIENT'
        FOR UPDATE
      `);
      if (membership.rows.length === 0) return undefined;
      const [walletTx] = await trx.insert(walletTransactions).values({
        userId,
        type: "CREDIT",
        amountCents,
        description,
