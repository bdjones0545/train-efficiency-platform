...
export async function validateAgentMailFollowupSchema(executor: Executor = db): Promise<void> {
  ...
  for (const expectedIndex of [
    ["agent_mail_followups_pkey",true,["id"],null],
    ["agent_mail_followups_org_id_id_unique",true,["organization_id","id"],null],
    ["agent_mail_followups_sequence_step_unique",true,["organization_id","source_reply_queue_id","sequence_step"],expectedPredicate],
    ["idx_followup_org_status_scheduled",false,["organization_id","status","scheduled_for"],null],
    ["idx_followup_inbox",false,["organization_id","inbox"],null],
    ["idx_followup_inbound",false,["organization_id","source_inbound_message_id"],null],
  ] as const) {
    const [name, unique, columnsWanted, predicate] = expectedIndex;
    const actual = indexes.find(index => index.index_name === name);
    if (!actual || actual.is_unique !== unique || !actual.is_valid || !actual.is_ready
