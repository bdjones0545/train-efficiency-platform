...
test("migration readiness exposes only expected/applied identifiers and state", async () => {
  ...
  assert.equal(state.latestExpected, "0022_wallet_redemption_idempotency_indexes.sql");
  assert.equal(state.latestApplied, "0022_wallet_redemption_idempotency_indexes.sql");
