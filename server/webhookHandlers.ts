  ...
  static async processWebhook(payload: Buffer, signature: string): Promise<void> {
    ...
    try {
      ...
      if (eventType === 'payment_intent.succeeded') {
        ...
        // Subscription-originated PIs always have pi.invoice set (Stripe populates
        // it for every invoice-generated charge). Crediting the wallet for these
        // payments is wrong: the value is tracked as user_subscriptions.sessionsRemaining,
        // not as a dollar balance. Wallet credit is handled for one-time payments only.
        const isSubscriptionPayment = !!pi.invoice;
        if (isSubscriptionPayment) {
          console.log(JSON.stringify({
            ...
            invoice: typeof pi.invoice === 'string' ? pi.invoice : (pi.invoice as any)?.id,
            message: 'Subscription payment — skipping wallet credit; sessionsRemaining managed via checkout.session.completed / invoice.paid',
            ts: new Date().toISOString(),
          }));
          logWebhookEvent({ eventId, eventType, livemode, paymentIntentId: pi.id, credited: false });
          await markWebhookEventDone(rowId, 'succeeded');
          return;
        }
      ...
      if (eventType === 'charge.succeeded') {
        ...
        // ── Subscription charge guard ──────────────────────────────────────────
        // Charges generated by a Stripe subscription always have charge.invoice set.
        // Wallet credit must not be written for these — value is in sessionsRemaining.
        const chargeIsSubscriptionPayment = !!charge.invoice;
        if (chargeIsSubscriptionPayment) {
          console.log(JSON.stringify({
            system: 'stripe_webhook',
            event: 'charge.succeeded',
            paymentModel: 'subscription',
            chargeId: charge.id,
            invoice: typeof charge.invoice === 'string' ? charge.invoice : (charge.invoice as any)?.id,
            message: 'Subscription charge — skipping wallet credit; sessionsRemaining managed via checkout.session.completed / invoice.paid',
            ts: new Date().toISOString(),
          }));
          logWebhookEvent({ eventId, eventType, livemode, credited: false });
          await markWebhookEventDone(rowId, 'succeeded');
          return;
        }
