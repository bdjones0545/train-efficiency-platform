...
- **Decision-Maker Contact Layer:** Lead research agent actively searches for decision-makers. Contact quality: `decision_maker` > `role_based` > `general` > `missing`. `POST /api/team-training-leads/:id/enrich-contact` runs enrichment per-lead.
- **Live Web Search Enrichment:** `enrichProspectContact` uses OpenAI Responses API (`gpt-4o` + `web_search_preview` tool) so the model actually browses the web in real-time to find source-backed emails. Falls back to chat completions if the Responses API is unavailable. No guessing or inference allowed — `foundRealEmail` must be true for any email to be saved.
- **Contact Evidence Layer:** Every discovered email persists `contactSourceTitle`, `contactSourceSnippet`, `contactDiscoveryMethod`, `contactConfidenceScore` (0.00–1.00), `contactDiscoveredAt`, `lastDiscoveryAttemptAt`, `lastDiscoveryResult`. Frontend shows a "Contact Evidence" panel with source link, snippet, method, confidence badge (green/yellow/red), and stale warning (>90 days).
- **Kevin Integration (Phases 3–8):** AI orchestration layer connecting TE to the Kevin external agent system. 9 DB tables (`kevin_*`), circuit breaker, navigation registry, capability matrix (6 modes), signal intake via `POST /api/internal/kevin/signals` (machine-to-machine, `TE_INTERNAL_SERVICE_TOKEN` bearer only), event dispatch queue with retry/dead-letter, context retrieval wired into CEO Heartbeat, and outcome forwarding closing the learning loop. All 3 data-plane endpoints (`/v1/events`, `/v1/context/query`, `/v1/outcomes`) are implemented but pending Hermes-side activation. Feature flags: `KEVIN_INTEGRATION_ENABLED`, `KEVIN_CONTEXT_RETRIEVAL_ENABLED`, `KEVIN_EVENT_DISPATCH_ENABLED`, `KEVIN_OUTCOME_FORWARDING_ENABLED`, `KEVIN_SIGNAL_INTAKE_ENABLED`. Migration: `migrations/0001_kevin_tables.sql` (idempotent). Docs: `docs/kevin-integration.md`.
